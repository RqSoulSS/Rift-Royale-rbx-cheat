--[[
    ...
    made by mikeymikey @ v3rm and MikeyA#5511 @ discord
...
local function getPlayerHealth(rig)
    ...
    if plr then
        return plr:GetAttribute("health") + plr:GetAttribute("shield"), plr:GetAttribute("maxHealth") + plr:GetAttribute("maxShield"), plr:GetAttribute("maxHealth"), plr:GetAttribute("maxShield")
    end
end
...
local function ConstructUI()
    ...
    Cache.UI.Credits.Size = textsize/1.25
    Cache.UI.Credits.Position = Vector2.new(1, Camera.ViewportSize.Y/2-200 - (Camera.ViewportSize.Y/2-200)/20)
...
local function DeconstructUI()
    for _,object in pairs(Cache.UI) do
...
Cache.Connections.MouseDown = Mouse.Button1Down:Connect(function()
    hoveringESP = checkInRange(Vector2.new(0, Camera.ViewportSize.Y/2-(200-(((Camera.ViewportSize.Y/2-200)/20)*1))-35), Vector2.new(Camera.ViewportSize.X/8-3, (Camera.ViewportSize.Y/2-200)/20))
    if hoveringESP then
    ...
    hoveringTracers = checkInRange(Vector2.new(0, Camera.ViewportSize.Y/2-(200-(((Camera.ViewportSize.Y/2-200)/20)*2))-35), Vector2.new(Camera.ViewportSize.X/8-3, (Camera.ViewportSize.Y/2-200)/20))
    ...
    hoveringAimbot = checkInRange(Vector2.new(0, Camera.ViewportSize.Y/2-(200-(((Camera.ViewportSize.Y/2-200)/20)*3))-35), Vector2.new(Camera.ViewportSize.X/8-3, (Camera.ViewportSize.Y/2-200)/20))
    ...
    hoveringAimbotFOV = checkInRange(Vector2.new(0, Camera.ViewportSize.Y/2-(200-(((Camera.ViewportSize.Y/2-200)/20)*5))-35), Vector2.new(Camera.ViewportSize.X/8-3, (Camera.ViewportSize.Y/2-200)/20))
    ...
    hoveringAimbotPower = checkInRange(Vector2.new(0, Camera.ViewportSize.Y/2-(200-(((Camera.ViewportSize.Y/2-200)/20)*4))-35), Vector2.new(Camera.ViewportSize.X/8-3, (Camera.ViewportSize.Y/2-200)/20))
    ...
    hoveringItemESP = checkInRange(Vector2.new(0, Camera.ViewportSize.Y/2-(200-(((Camera.ViewportSize.Y/2-200)/20)*6))-35), Vector2.new(Camera.ViewportSize.X/8-3, (Camera.ViewportSize.Y/2-200)/20))
    ...
    hoveringExit = checkInRange(Vector2.new(0, Camera.ViewportSize.Y/2-(200-(((Camera.ViewportSize.Y/2-200)/20)*19))-35), Vector2.new(Camera.ViewportSize.X/8-3, (Camera.ViewportSize.Y/2-200)/20))
    if hoveringExit then
        DestroyScript()
    end

    hoveringKeybind = checkInRange(Vector2.new(0, Camera.ViewportSize.Y/2-(200-(((Camera.ViewportSize.Y/2-200)/20)*18))-35), Vector2.new(Camera.ViewportSize.X/8-3, (Camera.ViewportSize.Y/2-200)/20))
...
local function getNear()
    ...
    for i,v in pairs(game.Workspace:GetChildren()) do
        if v.Name == "R15Rig" and v ~= game.Players.LocalPlayer.Character and not friendly(v) then
            local ScreenPosition, visible = Camera:WorldToViewportPoint(v.Head.Position)
            --local CF = CFrame.new(Camera.CameraSubject.Parent.HumanoidRootPart.Position, v.HumanoidRootPart.Position)
            --local hitt = workspace:Raycast(CF.p, CF.LookVector * (Camera.CameraSubject.Parent.HumanoidRootPart.Position - v.HumanoidRootPart.Position).magnitude, RayParams)
            if visible then
...
Cache.Connections.InputBegin = UserInputService.InputBegan:Connect(function(input)
    ...
    if input.UserInputType == Enum.UserInputType.MouseButton2 then
        ...
        repeat
            ...
            if player then
                magnitudeX = ((Cache.Enabled.AimbotPower/100)/6)*(-(Mouse.X-screenpos.X))
                magnitudeY = ((Cache.Enabled.AimbotPower/100)/6)*(-(Mouse.Y+30 - screenpos.Y))
...
local function ESPOnRig(Rig)
    if Rig.Name ~= "R15Rig" then return end
    ...
    TABLE.tracers.Color = Color3.new(0.631372, 0.141176, 0.141176)
...
for i,v in pairs(game.Workspace:GetChildren()) do
    if v.Name == "R15Rig" and v ~= game.Players.LocalPlayer.Character and v ~= Camera.CameraSubject.Parent then
        ESPOnRig(v)
    end
end
...
local function update()
    ...
    for rig, drawings in pairs(Cache.Players) do
        ...
        if not rig or rig.Parent == nil or (Camera.CameraSubject and rig == Camera.CameraSubject.Parent) then
            ESPBox:Remove() 
            ESPTracers:Remove()
        ...
        local health, maxhealth = getPlayerHealth(rig)
        local cframe = workspace.GetPivot(rig)
        local position, visible = WTVP(Camera, cframe.Position)
        if friendly(rig) then ESPBox.Color = Color3.new(0.058823, 0.545098, 1) ESPTracers.Color = Color3.new(0.058823, 0.545098, 1) else
        ...
        ESPBox.Visible = visible and Cache.Enabled.ESP
        ESPTracers.Visible = visible and Cache.Enabled.Tracers
        if visible then
...
local function update2()
    ...
    for part, draws in pairs(Cache.Items) do
        ...
        elseif part:GetAttribute("itemType") == "ak_5" or part:GetAttribute("itemType") == "deagle_5" or part:GetAttribute("itemType") == "barrett_5" then
        ...
        if not (part:GetAttribute("itemType") == "barrett_5" or part:GetAttribute("itemType") == "ak_4" or part:GetAttribute("itemType") == "ak_5" or part:GetAttribute("itemType") == "heavy_armor" or part:GetAttribute("itemType") == "deagle_5" or part:GetAttribute("itemType") == "launch_pad") then
            Box.Visible = false
            Text.Visible = false
        end
